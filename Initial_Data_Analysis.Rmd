...
Green_Weibull_model <- gamlss(formula = Green_Density ~ Time_Point*Treatment + random(Species_ID) + random(Field_Site) + random(Acclimation_Period), family = WEI(), data = Green_Symbionts, control = gamlss.control(n.cyc = 8))
summary(Green_Weibull_model)

#Dino
Dino_Symbionts$orderTreatment = ordered(Dino_Symbionts$Treatment, levels = c("Control", "25C", "30C"))

Dino_Gamma_model <- gamlss(formula = Dino_Density ~ Time_Point*Treatment + random(Species_ID) + random(Field_Site) + random(Acclimation_Period), family = GA(), data = Dino_Symbionts, control = gamlss.control(n.cyc = 3))
