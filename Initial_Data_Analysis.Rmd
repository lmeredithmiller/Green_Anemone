...
#Green
DeltaGreen_fist <- fitDist(Green_Density_Scaled, data = Delta_Data_model, type = "realline", try.gamlss = T)
DeltaGreen_hist <- histDist(Delta_Data_model$Green_Density_Scaled, "PE", density = F, main = "Power Exponential")

DeltaGreen_model <- gamlss(formula = Green_Density_Scaled ~ Date*Treatment + random(Species_ID) + random(Field_Site), family = PE(), method = RS(), data = Delta_Data_model, control = gamlss.control(n.cyc = 250))
summary(DeltaGreen_model)

#Dino
DeltaDino_fist <- fitDist(Dino_Density_Scaled, data = Delta_Data_model, type = "realline", try.gamlss = T)
DeltaGreen_hist <- histDist(Delta_Data_model$Dino_Density_Scaled, "SHASHo2", density = F, main = "SHASHo2")

DeltaDino_model <- gamlss(formula = Dino_Density_Scaled ~ Date*Treatment + random(Species_ID) + random(Field_Site), family = SHASHo2(), data = Delta_Data_model, control = gamlss.control(n.cyc = 250))
...
bartlett.test(Feeding_Time_Min ~ Treatment, data = Feeding_Data) # p-value > 0.05, equal variance 
```
