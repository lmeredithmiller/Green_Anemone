...
### Density Data
The two datasets below are used in models for algae density of zoochlorellae and zooxathellae. It filters out unwanted columns and drops NAs for Green_Density and Dino_Density.
...
gamma_test(Green_Density_data$Green_Density)       #p-value > 0.05, larger i can use, p = 0.123
...
weibull_test(Green_Density_data$Green_Density)     #p-value > 0.05, larger i can use, p = 0.44
...
gamma_test(Dino_Density_data$Dino_Density)         #p-value > 0.05, larger i can use, p = 0.6709
...
weibull_test(Dino_Density_data$Dino_Density)       #p-value > 0.05, larger i can use, p = 0.774
...
```{r}
#Green
Green_log <- Green_Density_data %>% mutate(log_data = log(abs(Green_Density + 0.01)))

shapiro.test(Green_log$log_data)      #p-value < 0.05, not normal distribution

#Dino
Dino_log <- Dino_Density_data %>% mutate(log_data = log(abs(Dino_Density + 0.01)))
...
```{r}
#Green
Green_gaussian <- Green_Density_data %>% mutate(gaus_data = Gaussianize(Green_Density_data$Green_Density, 's'))
shapiro.test(Green_gaussian$gaus_data)      #p-value < 0.05, not normal distribution, p = 3.19e-05

#Dino
Dino_gaussian <- Dino_Density_data %>% mutate(gaus_data = Gaussianize(Dino_Density_data$Dino_Density, 's'))
...
Plots:
```{r, warning=FALSE, message=FALSE}
green_plot <- ggplot(filtered_dates, aes(x = Date, y = Green_Density)) +
  geom_boxplot() +
  xlab("Event") +
...
```{r, warning=FALSE, message=FALSE}
dino_plot <- ggplot(filtered_dates, aes(x = Date, y = Dino_Density)) +
  geom_boxplot() +
  xlab("Event") +
...
Feeding_Data <- filtered_dates %>% 
  filter(Date != "11/13/2021") %>% 
...
### Ordinal Regression
```{r}
as_model = clmm(Open_Closed ~ Bucket_Temp * Sym.Status + (1 | system) + (1 | Genotype), data = heat_data)
```
