...
#Greens
Green_Symbiont_Visual <- fitDist(Green_Density, data = filtered_dates, type = "realAll", try.gamlss = T)

histDist(y, family = SHASH, density = FALSE, 
        nbins = 10, xlab = "x", ylab = "y", data = Green_Symbiont_Visual, 
        col.hist = "gray", border.hist = "blue", 
        fg.hist = rainbow(12)[9])

#Dinos
Dino_Symbiont_Visual <- fitDist(Dino_Density, data = filtered_dates, type = "realAll", try.gamlss = T)

histDist(y, family = SHASH, density = FALSE, 
        nbins = 10, xlab = "x", ylab = "y", data = Dino_Symbiont_Visual, 
        col.hist = "gray", border.hist = "blue", 
        fg.hist = rainbow(12)[9])
```
...
#Greens
Green_Symbiont_Model <- gamlss(formula = Green_Density ~ Time_Point*Treatment + random(Species_ID), family = SHASH(), data = Density_data, control = gamlss.control(n.cyc = 250))
#not converged by 250

summary(Green_Symbiont_Model)

#Dinos
Dino_Symbiont_Model <- gamlss(formula = Green_Density ~ Time_Point*Treatment + random(Species_ID), family = SHASH(), data = Density_data, control = gamlss.control(n.cyc = 250))
#not converged by 250

summary(Dino_Symbiont_Model)
```
