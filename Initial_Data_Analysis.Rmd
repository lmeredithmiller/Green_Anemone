...
PAM_data <- health_data %>% 
  select(-c(Field_Site, Acclimation_Period, Base_Width, Base_Length, Base_Diagonal, Base_Diameter_mm, Nb_Tentacles, 
            ...
            Feeding_Time_Min, Feeding_Time, Feeding_Start, Feeding_Stop, Photo_ID, Removed, MI_Green, MI_Dino, Fv_Fm_1, 
            Fv_Fm_2, Fv_Fm_3)) %>% 
  drop_na(PAM_avg) %>% 
  drop_na(Event_True) %>% 
  drop_na(Time_Point) %>% 
...
Green_Density_data <- health_data %>% 
  ...
  drop_na(Time_Point) %>% 
  mutate(Green_Density + 1) %>% 
  mutate(Dino_Density + 1) %>% 
  filter(Green_Density != 0)

#right transform 
green_right_transform <- rightSkew %>%
  mutate(Green_Density_data = log(abs(values)))

ggplot(green_right_transform, aes(x = Green_Density_data)) + 
  geom_density() + 
  theme_classic() + 
  geom_vline(xintercept = 0, colour = "red")

#check with shapiro
shapiro.test(green_right_transform$Green_Density_Data)

#left transform 
green_left_transform <- leftSkew %>%
  mutate(Green_Density_data = sqrt(abs(values)))

ggplot(left_transform, aes(x = Green_Density_data)) + 
  geom_density() + 
  theme_classic() + 
  geom_vline(colour = "red")

#check with shapiros
shapiro.test(green_left_transform$Green_Density_data)
