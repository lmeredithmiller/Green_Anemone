...
health_data <- health_data %>% 
  ...
  mutate(Treatment = fct_relevel(Treatment, "Control", "25C", "30C"), Event = fct_relevel("Acclimation", "Pre-heat", "Post-heat", "Recovery"))
...
```{r, warning=FALSE, message=FALSE}
exp_test(health_data$PAM_avg)        #p-value < 0.05, cannot use
gamma_test(health_data$PAM_avg)      #p-value < 0.05, cannot use
lnorm_test(health_data$PAM_avg)      #p-value < 0.05, cannot use
normal_test(health_data$PAM_avg)     #p-value < 0.05, cannot use
weibull_test(health_data$PAM_avg)    #p-value < 0.05, cannot use
```
...
```{r, warning=FALSE, message=FALSE}
 ggplot(filter(PAM_data, Event == "Pre-heat" | Event == "Post-heat"), aes(x = Event, y = PAM_avg)) +
  geom_boxplot() +
