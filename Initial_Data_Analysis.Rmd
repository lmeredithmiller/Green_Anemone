...
size_plot <- ggplot(data = Size_Data, aes(x=Date, y = Mean_Size, colour = Treatment, group = Treatment)) +
  theme_test() +
  geom_point(position = position_dodge(width = 0.1)) +
  geom_linerange(aes(ymin = Mean_Size - SE_Size, ymax = Mean_Size + SE_Size), position = position_dodge(width = 0.1)) +
  geom_line(position = position_dodge(width = 0.1)) +
  labs(x="Event", 
...
behaviour_heat <- behaviour_heat + plot_layout(ncol=2)
behaviour_heat
