...
health_data <- health_data %>% 
  ...
  mutate(Total_Density = Green_Density + Dino_Density) %>% 
...
```{r}
ggplot(Green_Symbionts, aes(x = Date, y = log(Total_Density), fill = Treatment)) +
  geom_boxplot()
```
...
# #family SHASHo
# MIDino_hist <- histDist(MI_Dino_scaled$MI_Dino_scaled, "SHASHo", density = T, main = "SHASHo")

```

Dunn Test on MI Data:
```{r}
dunnTest(MI_Green ~ Treatment, data = MI_Data)
kruskal.test(MI_Green ~ Treatment, data = MI_Data)

dunnTest(MI_Dino ~ Treatment, data = MI_Data)
kruskal.test(MI_Dino ~ Treatment, data = MI_Data)
```
Dunn test results for Green:
25C - 30C: Z = -3.036/ Padju = 0.007182786	
25C - Control: Z = -1.4418/ Padju = 0.149347449	
30C - Control: Z = 1.7345/ Padju = 0.165644824
Kruskal test results for Green:
chi-squared = 9.492, df = 2, p = 0.008686
Dunn test results for Dino:
25C - 30C: Z = -0.072756/ Padju = 0.9420003	
25C - Control: Z = -0.60896/ Padju = 1.0000000	
30C - Control: Z = -0.6185/ Padju = 1.0000000	
Kruskal test results for Dino:
chi-squared = 0.51372, df = 2, p = 0.7735
...
dino_MI_plot <- ggplot(Green_Symbionts, aes(x = Date, y = MI_Dino)) +
  geom_boxplot() +
  xlab("Event") +
  ylab("Mitotic index") +
  facet_grid(. ~ Treatment) +
...
```{r}
green_MI_plot <- ggplot(Green_Symbionts, aes(x = Date, y = MI_Green, fill = Treatment)) +
  geom_boxplot(alpha = 0.85) +
  xlab("Event") +
  ...
  scale_fill_manual(limits = c("Control", "25C", "30C"),
                    ...
                    values = cols_for_heat) +
  theme_test()
green_MI_plot <- green_MI_plot + ylim(0,60) + scale_x_discrete(labels=c("11/5/2021" = "Pre-heat", "11/9/2021" = "Post-heat",
                              "11/13/2021" = "Recovery"))

dino_MI_plot <- ggplot(Dino_Symbionts, aes(x = Date, y = MI_Dino, fill = Treatment)) +
  geom_boxplot(alpha = 0.85) +
  xlab("Event") +
  ...
  scale_fill_manual(limits = c("Control", "25C", "30C"),
                    ...
                    values = cols_for_heat) +
  theme_test()
dino_MI_plot <- dino_MI_plot + ylim(0,30) + scale_x_discrete(labels=c("11/5/2021" = "Pre-heat", "11/9/2021" = "Post-heat",
                              "11/13/2021" = "Recovery"))
...
PAM_over_time <- ggplot(data = filtered_PAM_data, aes(x=Date, y = Mean_PAM, colour = Treatment, group = Treatment)) +
  ...
  scale_colour_manual(limits = c("Control", "25C", "30C"),
                    ...
                    values = cols_for_heat) +
  theme_test()
PAM_over_time <- PAM_over_time + scale_x_discrete(labels=c("11/6/2021" = "Pre-heat", "11/9/2021" = "Post-heat", "11/13/2021" = "Recovery"))
PAM_over_time
...
Size_Data <- health_data %>% 
  ...
  filter(Date == "11/5/2021" | Date == "11/9/2021" | Date == "11/13/2021") %>% 
  mutate(Date = fct_relevel(Date, "11/5/2021", "11/9/2021", "11/13/2021"), (Treatment = fct_relevel(Treatment, "Control", "25C", "30C"))) %>% 
...
```
The control and 25 did have significant values, but the 30 did not. Did they grow in control and 25 but not 30 which means they could be stressed out at 30 and not in control or 25 at the end of recovery?
...
```{r}
size_plot <- ggplot(data = Size_Data, aes(x=Date, y = Mean_Size, colour = Treatment, group = Treatment)) +
  geom_point(position = position_dodge(width = 0.1)) +
  geom_linerange(qaes(ymin = Mean_Size - SE_Size, ymax = Mean_Size + SE_Size), position = position_dodge(width = 0.1)) +
  geom_line(position = position_dodge(width = 0.1)) +
  labs(x="Event", 
  ...
  scale_colour_manual(limits = c("Control", "25C", "30C"),
                    ...
                    values = cols_for_heat) +
  theme_test()
size_plot <- size_plot + scale_x_discrete(labels=c("11/5/2021" = "Pre-heat", "11/9/2021" = "Post-heat", "11/13/2021" = "Recovery"))
size_plot
